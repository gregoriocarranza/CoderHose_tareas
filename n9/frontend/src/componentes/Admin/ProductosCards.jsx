...
const ProductosCards = (prop) => {
  ...
  const [checked, setChecked] = useState(task.Mostrar_Web);

  const updFunction = async (e,checkedds) => {
    if (checkedds==1) {
      console.log(e.target.checked);
      setToUpdate({ ...toUpdate, id: task.id, [e.target.name]: e.target.checked });
